...
// Recreate __dirname for ES modules (Required to serve the frontend)
...
// This tells Express to serve the built frontend files from the 'dist' folder
...
app.post("/api/admin/products", auth, (req, res) => {
  ...
  const r = db.prepare("INSERT INTO products(name,composition,dosage_form,category,image_url,description) VALUES(?,?,?,?,?,?)").run(p.name, p.composition || "", p.dosage_form || "", p.category || "General", p.image_url || "/products/product-placeholder.svg", p.description || "");
...
app.put("/api/admin/products/:id", auth, (req, res) => {
  const p = req.body || {};
  db.prepare("UPDATE products SET name=?,composition=?,dosage_form=?,category=?,image_url=?,description=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(p.name, p.composition, p.dosage_form, p.category, p.image_url, p.description, p.active ? 1 : 0, req.params.id);
...
app.patch("/api/admin/enquiries/:id", auth, (req, res) => {
  ...
  db.prepare("UPDATE enquiries SET status=? WHERE id=?").run(status || "New", req.params.id);
...
// If no API route matches, send the frontend's index.html
// This allows React Router / Vue Router to work correctly
app.get(/.*/, (req, res) => {
